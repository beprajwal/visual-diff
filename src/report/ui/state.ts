...
import { buildFilmstrip, runIndex, viewportsOf, visibleCells } from './derive.js';
...
function withPair(state: AppState, base: RunId, head: RunId): AppState {
  const newest = newestRunId(state.runs);
...
function shiftHead(state: AppState, delta: number): AppState {
  const index = runIndex(state.runs, state.head);
  if (index < 0) return state;
  const nextIndex = index + delta;
  const next = state.runs[nextIndex];
  if (!next) return state;
  // The base follows one behind, which is the pair a reviewer stepping through iterations wants.
  const base = state.runs[nextIndex - 1] ?? next;
...
export function reduce(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case 'runs-loaded': {
      ...
      const known = (id: RunId | null): boolean => runIndex(runs, id) >= 0;
      if (known(state.base) && known(state.head)) {
        const newest = newestRunId(runs);
    ...
    case 'select-head': {
      ...
      const index = runIndex(state.runs, action.head);
      const base = index > 0 ? state.runs[index - 1] : undefined;
    ...
    case 'jump-to-pending': {
      ...
      const index = runIndex(state.runs, pending.runId);
      const base = index > 0 ? state.runs[index - 1] : undefined;
      return {
...
function reduceServerEvent(state: AppState, event: ServerEvent): AppState {
  switch (event.type) {
    ...
    case 'run': {
      ...
      const base =
        previousHead !== null && isAdjacentPair(previousRuns, state.base, previousHead)
          ? previousHead
